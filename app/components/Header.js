...
const Header = () => {
    ...
    const { data: session, status } = useSession(); // Mendapatkan status login

    // Fungsi untuk menutup dropdown saat klik di luar menu
    useEffect(() => {
    ...
    if (status === "loading") {
        return <header className="bg-blue-700 text-white h-12 flex items-center justify-between px-6 fixed top-0 w-full shadow-md z-50">
            {/* ✅ Judul Header Sekarang Bisa Diklik untuk Navigasi ke Home */}
            <h1
    ...
    return (
        <header className="bg-blue-700 text-white h-12 flex items-center justify-between px-6 fixed top-0 w-full shadow-md z-50">
            ...
            {!session ? (
                <>
                    {/* Tengah: Dropdown & Inquiry Aplikasi */}
...
// Komponen Dropdown dengan kategori dalam URL
const Dropdown = ({ title, isOpen, toggle, items, category, alignRight, router }) => (
    <div className="relative dropdown-container">
        ...
        {isOpen && (
            <div className={`absolute mt-2 w-56 bg-white text-black rounded shadow-lg max-h-64 overflow-auto
                ${alignRight ? "right-0" : "left-0"}`}>
                {items.map((item, index) => (
                    <DropdownItem key={index} text={item} category={category} router={router} toggleDropdown={toggle} />
                ))}
            </div>
        )}
    </div>
...
// Komponen Dropdown Item dengan Routing Kategori
const DropdownItem = ({ text, category, router, toggleDropdown }) => {
    const formattedPath = `/${category}/${text.toLowerCase().replace(/\s+/g, "-")}`; // ✅ Ubah spasi jadi "-", tambahkan kategori
    return (
        <a
            onClick={() => {
                router.push(formattedPath); // Navigasi ke halaman
                toggleDropdown(); // Menutup dropdown setelah item dipilih
            }}
