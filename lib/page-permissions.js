...
const ALLOWED_CACHE_SYMBOL = Symbol.for("reportff.allowedPagePermissionsCache");
const ALLOWED_CACHE_TTL_MS = 60 * 1000;
...
function getAllowedCacheStore() {
  if (!globalThis[ALLOWED_CACHE_SYMBOL]) {
    globalThis[ALLOWED_CACHE_SYMBOL] = new Map();
  }
  return globalThis[ALLOWED_CACHE_SYMBOL];
}
...
export function invalidatePagePermissionsCache() {
  ...
  cache.data = null;
  cache.expiresAt = 0;
  invalidateAllowedPermissionsCache();
}

export function invalidateAllowedPermissionsCache() {
  const store = getAllowedCacheStore();
  store.clear();
}
...
export async function createPagePermission({
  ...
  const data = await fetchPermissionById(insertedId);
...
export async function updatePagePermission(
  ...
  const data = await fetchPermissionById(id);
  invalidatePagePermissionsCache();
  return data;
}
...
function buildAllowedCacheKey(userId, groupIds, divisionId) {
  const normalizedGroups = Array.isArray(groupIds)
    ? groupIds.map((value) => String(value)).sort()
    : [];
  const divisionKey =
    divisionId === null || divisionId === undefined ? "" : String(divisionId);
  return `${userId ?? "anonymous"}|${divisionKey}|${normalizedGroups.join(
    ","
  )}`;
}

function readAllowedCache(key) {
  const store = getAllowedCacheStore();
...
function writeAllowedCache(key, value) {
  const store = getAllowedCacheStore();
  store.set(key, {
    value,
...
export async function getAllowedPagePermissionsForUser({
  userId = null,
  groupIds = [],
  divisionId = null,
} = {}) {
  const cacheKey = buildAllowedCacheKey(userId, groupIds, divisionId);
  const cached = readAllowedCache(cacheKey);
  if (cached !== undefined) {
    return cached;
  }
  ...
  const divisionKey =
    divisionId === null || divisionId === undefined
      ? null
      : String(divisionId);
  ...
  const allowed = permissions.filter((permission) => {
    ...
    const groupsAllowed =
      permission.groupIds?.some((id) => groupSet.has(String(id))) ?? false;
    ...
    return (
      permission.divisionIds?.some(
        (id) => String(id) === divisionKey
      ) ?? false
    );
  });

  writeAllowedCache(cacheKey, allowed);
  return allowed;
}
