CREATE OR REPLACE PROCEDURE ILOS.griya_pending_progress_export (
...
SELECT
      ...
      CASE
        ...
        WHEN b.LVL = ''3'' THEN (SELECT c.BRANCH_CODE || '' - '' || c.NAME FROM ILOS.TBL_BRANCH c WHERE c.BRANCH_CODE=b.PARENT_ID)
        ELSE ''''
      END AS NAMA_AREA,
      
      CASE
        WHEN b.LVL = ''2'' THEN (SELECT c.BRANCH_CODE || '' - '' || c.NAME FROM ILOS.TBL_BRANCH c WHERE c.BRANCH_CODE=b.PARENT_ID)
        WHEN b.LVL = ''3'' THEN (SELECT c.BRANCH_CODE || '' - '' || c.NAME FROM ILOS.TBL_BRANCH c WHERE c.BRANCH_CODE=b.PARENT_ADMINISTRASI)
        ELSE ''''
      END AS REGION,

      CASE
        WHEN (a.JENIS_PRODUK LIKE ''%PENSIUN%'') THEN 
            (SELECT (c.EVENT_NO || '' - '' || c.EVENT_NAME) FROM ILOS.TBL_EVENT_PENSIUN c WHERE c.EVENT_NO=ILOS.f_get_value_clob(a.DATA,''kodeProgram''))
        WHEN (a.JENIS_PRODUK LIKE ''%MITRAGUNA%'') THEN
            (SELECT (c.EVENT_NO || '' - '' || c.EVENT_NAME) FROM ILOS.TBL_EVENT_IMPLAN c WHERE c.EVENT_NO=ILOS.f_get_value_clob(a.DATA,''kodeProgram''))
        WHEN (a.JENIS_PRODUK LIKE ''%GRIYA%'') THEN
          (SELECT (c.EVENT_NO || '' - '' || c.EVENT_NAME) FROM ILOS.TBL_EVENT c WHERE c.EVENT_NO=ILOS.f_get_value_clob(a.DATA,''kodeProgram''))
      END AS EVENT,
      ...
      (SELECT us.BRANCH_CODE FROM ILOS.TBL_APLIKASI_HIST h 
      INNER JOIN ILOS.TBL_USER us ON us.USER_ID = h.UPDATE_BY 
      WHERE h.NO_APLIKASI=a.NO_APLIKASI AND h.FLOW_CODE LIKE ''%STAGE-DDE%'' 
      FETCH FIRST 1 ROWS ONLY) AS BRANCH_DDE
    ...
    WHERE (a.JENIS_PRODUK LIKE ''%GRIYA%'') AND (a.FLOW_CODE NOT LIKE ''%_HOLD%'') '|| v_where_tgl || v_where_area||'
    ';
