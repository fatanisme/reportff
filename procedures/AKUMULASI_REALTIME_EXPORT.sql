CREATE OR REPLACE PROCEDURE ILOS.akumulasi_realtime_export (
...
IS
    v_sql CLOB;
   	v_where_tgl CLOB := '';
BEGIN
	v_where_tgl := 'AND (SUBSTR(a.NO_APLIKASI,1,8) >= ''' || p_tglawal || ''' AND SUBSTR(a.NO_APLIKASI,1,8) <= ''' || p_tglakhir || ''')';

    -- Bangun SQL utama
    v_sql := '
SELECT
        
    to_char((SELECT h.CREATE_DATE FROM ILOS.TBL_APLIKASI_HIST h 
	WHERE h.NO_APLIKASI=a.NO_APLIKASI AND h.FLOW_CODE LIKE ''%STAGE-IDE%'' 
	ORDER BY h.CREATE_DATE DESC FETCH FIRST 1 ROWS ONLY),''yyyy-mm-dd'') AS TGL_INPUT,
  ...
  INNER JOIN ILOS.TBL_USER usr ON usr.USER_ID = a.CREATE_BY
    ...
    (a.FLOW_CODE NOT LIKE ''%_HOLD%'') '|| v_where_tgl ||'
    ';
